...
x_test = read_mat.read_mat('../../data/test_data_100.mat')['test_data'] / 400.0
y_test = read_mat.read_mat('../../data/test_label_100.mat')['test_label'][0]

Y_test = dense_to_one_hot(y_test, n_classes=6)

position_test = hdf5storage.loadmat('../../data/test_position_100.mat')['test_position']
...
np.savetxt("../../result/ek2em/train/val_position.txt",position_test,fmt="%d")
...
for epoch_i in range(n_epochs):
    ...
    if epoch_i % 5 == 0:
        ...
        np.savetxt("../../result/unsp_bi/train_theta_result_"+str(epoch_i)+".txt",out_theta,fmt="%f")
        ...
        np.savetxt("../../result/unsp_bi/val_theta_result_"+str(epoch_i)+".txt",val_theta,fmt="%f")

        saver.save(sess, '../../model/unsp_bi/' + str(epoch_i) + '.tfmodel');
