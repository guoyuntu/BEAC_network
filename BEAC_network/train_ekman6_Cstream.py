...
from shuffle_data import shuffle, shuffle_100, shuffle_100_cnn
...
np.savetxt("/home/g_jiarui/video_spacial_tu/result/4/train/val_position.txt",position_val,fmt="%d")

#x_train_trans = np.transpose(x_train)

[X_train,y_train, position] = shuffle_100_cnn(x_train, y_train, position_train, 1080)
...
for epoch_i in range(n_epochs):
    ...
    acc_a = sess.run(accuracy,feed_dict={x: X_valid_a,y: Y_valid_a,keep_prob: 1.0})
    acc_su = sess.run(accuracy,feed_dict={x: X_valid_su,y: Y_valid_su,keep_prob: 1.0})
    acc_f = sess.run(accuracy,feed_dict={x: X_valid_f,y: Y_valid_f,keep_prob: 1.0})
    acc_j = sess.run(accuracy,feed_dict={x: X_valid_j,y: Y_valid_j,keep_prob: 1.0})
    acc_sa = sess.run(accuracy,feed_dict={x: X_valid_sa,y: Y_valid_sa,keep_prob: 1.0})
    acc_d = sess.run(accuracy,feed_dict={x: X_valid_d,y: Y_valid_d,keep_prob: 1.0})
    acc = str(sess.run(accuracy,feed_dict={x: X_valid,y: Y_valid,keep_prob: 1.0}))
