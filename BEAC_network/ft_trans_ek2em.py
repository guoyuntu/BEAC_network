...
x_train = np.load('../../data/multi_train_data_100.npy') / 400.0
...
[X_train,y_train, position] = shuffle_100(x_train, y_train, position_train, 2400)
...
#X_test = hdf5storage.loadmat('../../data/multi_test_data_100.mat')['test_data'][0]
#y_test = hdf5storage.loadmat('../../data/multi_test_label.mat')['test_label'][0]

Y_test = dense_to_one_hot(y_test, n_classes=6)

X_train = np.resize(X_train, (480, 100, 4096, 1))
X_test = np.resize(X_test, (600, 100, 4096, 1))
...
#saver.restore(sess, '../../model/beac_2/6.tfmodel')
...
for epoch_i in range(n_epochs):
    ...
    print('test acc(%d): %.4f'%(epoch_i, acc_test))
    #finetuning: epoch3 51.17% epoch5 65.33% epoch10 69.83%
    #without: epoch3 50.17% epoch5 58.67% epoch10 57.33%
