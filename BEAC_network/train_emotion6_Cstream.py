...
np.savetxt("/home/g_jiarui/video_spacial_tu/snythetic/color/result_10/1*5_initial=0.5/train/val_position.txt",position_val,fmt="%d")
...
gttheta1 = tf.placeholder(tf.float32, [31])
gttheta2 = tf.placeholder(tf.float32, [31])
...
out_theta = []
out_position = []
ff = open('/home/g_jiarui/video_spacial_tu/result/1/train/accuracy.txt','w+')
...
for epoch_i in range(n_epochs):
    for iter_i in range(iter_per_epoch - 1):
        ...
        if epoch_i > 10:
        	if cal_mean_tIou(theta,iter_i) < threshold:
        		...
        		print(cal_mean_tIou(theta,iter_i))
    ...
    if epoch_i % 5 == 1:
        ...
        np.savetxt("/home/g_jiarui/video_spacial_tu/result/1/rain_theta_result_"+str(epoch_i)+".txt",out_theta,fmt="%f")

        val_theta = sess.run(h_fc_loc2, feed_dict={
                x: X_valid, y: Y_valid, keep_prob: 1.0})

        np.savetxt("/home/g_jiarui/video_spacial_tu/result/1/val_theta_result_"+str(epoch_i)+".txt",val_theta,fmt="%f")
