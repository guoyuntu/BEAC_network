...
from cal_tIou_new import cal_mean_tIou, get_gt_position

x_train = read_mat.read_mat_v('/home/video_spacial_tu/data/multi_train_data.mat', 'train_data')/200
y_train = read_mat.read_mat('/home/video_spacial_tu/data/multi_train_label.mat')['train_label']

X_valid = read_mat.read_mat('/home/video_spacial_tu/data/multi_validation_data.mat')['validation_data']/200
y_valid = read_mat.read_mat('/home/video_spacial_tu/data/multi_validation_label.mat')['validation_label'][0]

X_test = read_mat.read_mat('/home/video_spacial_tu/data/multi_test_data.mat')['test_data']/200
y_test = read_mat.read_mat('/home/video_spacial_tu/data/multi_test_label.mat')['test_label'][0]

position_train = read_mat.read_mat('/home/video_spacial_tu/data/multi_train_position.mat')['train_position']
position_val = read_mat.read_mat('/home/video_spacial_tu/data/multi_validation_position.mat')['validation_position']
np.savetxt("/home/video_spacial_tu/result/1/train/val_position.txt",position_val,fmt="%d")
...
#h_flat = tf.reshape(h_trans, [-1, 10*4096])
...
out_theta = []
out_position = []
ff = open('/home/video_spacial_tu/result/1/train/accuracy.txt','w+')
...
for epoch_i in range(n_epochs):
    for iter_i in range(iter_per_epoch -1):
        ...
        if epoch_i > 5:
        	if cal_mean_tIou(theta,iter_i) < threshold:
        		ST_loss = sess.run(STloss,feed_dict={x: batch_xs,gttheta1: get_gt_position(iter_i)[:,0],gttheta2: get_gt_position(iter_i)[:,1],keep_prob: 1.0})
        		print('epoch_i: ' + str(epoch_i) + 'iter_i: ' + str(iter_i) +' ST_loss: ' + str(ST_loss))
        		print('mean_tIou: ',cal_mean_tIou(theta,iter_i))
        		sess.run(optimizer_ST,feed_dict={x: batch_xs,gttheta1: get_gt_position(iter_i)[:,0],gttheta2: get_gt_position(iter_i)[:,1],keep_prob: 0.75})
        		continue
    ...
    # theta = sess.run(h_fc1_loc, feed_dict={
    #     x: batch_xs, keep_prob: 1.0})
    # print theta
    ...
    if epoch_i % 5 == 1:
        ...
        np.savetxt("/home/video_spacial_tu/result/1/train_theta_result_"+str(epoch_i)+".txt",out_theta,fmt="%f")

        val_theta = sess.run(h_fc_loc2, feed_dict={
                x: X_valid,gttheta1: get_gt_position(iter_i)[:,0],gttheta2: get_gt_position(iter_i)[:,1], keep_prob: 1.0})

        np.savetxt("/home/video_spacial_tu/result/1/val_theta_result_"+str(epoch_i)+".txt",val_theta,fmt="%f")
