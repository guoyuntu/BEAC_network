...
def cal_mAP(url1, url2):
        ...
        for i in range(len(tiou)):
            ...
            if (tiou_1[i] < 0.2 and (tiou_2[i] < 0.2 or np.isnan(tiou_2[i])) and (tiou_3[i] < 0.2 or np.isnan(tiou_3[i]))) :
                fp[i] = 1
...
def cal_filter_mAP(url):
    ...
    for filename in filelist :
        if filename[-4:len(filename)] == '.txt' :
            ...
            mAP[num] = cal_mAP(url + '/position/' + filename, ground_url)
            num = num + 1
    return mAP
...
def cal_all_mAP():
    ...
    ff = open('synthetic_mAP_Ekman2.txt', 'w')
    ff.write(json.dumps(all_mAP))
    ff.close()
    #print all_mAP

def draw_mAP():
    ff = open('synthetic_mAP_Ekman2.txt', 'r')
    all_mAP = ff.read()
...
if __name__=='__main__':
    ...
    cal_all_mAP()
    draw_mAP()
    #print cal_mAP('/Users/mac/Desktop/Ekman/5*1_initial=0.2/position/train_theta_result_125.txt','/Users/mac/Desktop/Ekman/5*1_initial=0.2/train/train_position.txt')
