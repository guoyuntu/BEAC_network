...
x_train = read_mat.read_mat('../../data/train_data_100.mat')['train_data']
...
x_valid = read_mat.read_mat('../../data/validation_data_100.mat')['validation_data']
y_valid = read_mat.read_mat('../../data/validation_label_100.mat')['validation_label'][0]

Y_valid = dense_to_one_hot(y_valid, n_classes=6)

x_test = read_mat.read_mat('../../data/test_data_100.mat')['test_data']
...
np.savetxt("../../result/att_ek6/train/val_position.txt",position_test,fmt="%d")
...
out_theta = []
out_position = []
ff = open('../../result/att_ek6/train/accuracy.txt','w+')
print('finish building network')
#target_names = ['anger', 'suprise', 'fear', 'joy', 'sad', 'disgust']

saver.restore(sess, '../../model/att_ek6/10.tfmodel')
...
for epoch_i in range(n_epochs):
    ...
    if (epoch_i % 1 == 0):
        ...
        saver.save(sess, '../../model/att_ek6/' + str(epoch_i) + '.tfmodel');
