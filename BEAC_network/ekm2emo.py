...
x_train = read_mat.read_mat('../../data/train_data_100.mat')['train_data'] / 400.0
y_train = read_mat.read_mat('../../data/train_label_100.mat')['train_label'][0]
position_train = read_mat.read_mat('../../data/train_position_100.mat')['train_position']

[X_train,y_train, position] = shuffle_100(x_train, y_train, position_train, 1080)

Y_train = dense_to_one_hot(y_train, n_classes=6)

x_valid = read_mat.read_mat('../../data/validation_data_100.mat')['validation_data'] / 400.0
y_valid = read_mat.read_mat('../../data/validation_label_100.mat')['validation_label'][0]

Y_valid = dense_to_one_hot(y_valid, n_classes=6)

X_test = np.load('../../data/multi_train_data_100.npy') / 400
y_test = hdf5storage.loadmat('../../data/multi_train_label.mat')['train_label'][0]

position_val = hdf5storage.loadmat('../../data/multi_validation_position.mat')['validation_position']
...
X_test = np.resize(X_test, (2400, 100, 4096, 1))

np.savetxt("../../result/ek2em/train/val_position.txt",position_val,fmt="%d")
...
out_theta = []
out_position = []
ff = open('../../result/ek2em/train/accuracy.txt','w+')
print('finish building network')

saver.restore(sess, '../../model/unsp_bi/75.tfmodel')
...
for epoch_i in range(n_epochs):
    ...
    if epoch_i % 5 == 1:
        ...
        np.savetxt("../../result/ek2em/train_theta_result_"+str(epoch_i)+".txt",out_theta,fmt="%f")
        ...
        np.savetxt("../../result/ek2em/val_theta_result_"+str(epoch_i)+".txt",val_theta,fmt="%f")

        saver.save(sess, '../../model/beac_2/' + str(epoch_i) + '.tfmodel');
