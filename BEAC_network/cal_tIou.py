...
def get_gt_position(iter_i):
	g = open('/home/g_jiarui/video_spacial_tu/synthetic/color/result_10/1*5_initial=0.5/train/train_theta.txt', 'r')
	gtdata=g.read()
	
	gtdata = gtdata.split('\n')
	gtdata = gtdata[0:len(gtdata) - 1]
	
	ground = np.zeros([31, 2],dtype = float)
	
	num = 0
	for i in gtdata[31 * iter_i: 31 * iter_i + 31]:
		temp = i.split(' ')
...
def cal_mean_tIou(theta_result, iter_i):
    gt = get_gt_position(iter_i)
    
    ground = np.zeros([31, 2],dtype = int)
    ground[:,1] = (gt[:,0] + gt[:,1] + 1)* 15
    ground[:,0] = (gt[:,1] - gt[:,0] + 1)* 15
    
    position_result = np.zeros([31, 2],dtype = int)
    position_result[:,1] = (theta_result[:,0] + theta_result[:,1] + 1)* 15
    position_result[:,0] = (theta_result[:,1] - theta_result[:,0] + 1)* 15
